...
function StarknetChainIdComponent({ id }: { id: StarknetChainId }) {
  ...
  return (
    <div className="border border-borderGray uppercase text-right flex-row-reverse font-bold overflow-clip flex bg-white px-3 py-1 gap-6 items-center justify-between">
      <div>{displayName}</div>
...
function OtherChainIdComponent({ id }: { id: ChainId }) {
  return (
    <div className="border border-borderGray w-[136px] uppercase text-right overflow-clip font-bold flex px-3 py-1 items-center justify-end">
      <div>{id}</div>
