...
import { OptimizedDataTable } from "@/shared/components/optimized-data-table";
import {
  PageHeader,
  PageHeaderRight,
  PageHeaderTitle,
...
const Calldata = lazy(() => import("./calldata").then(m => ({ default: m.Calldata })));

/**
 * Convert raw value to STRK with 18 decimal places
 */
const ConvertToSTRK = (input: number | bigint) => {
...
const TransactionBreadcrumb = memo(({ txHash, isMobile }: { txHash?: string; isMobile: boolean }) => (
      ...
      <BreadcrumbItem>
        {txHash ? (
          <BreadcrumbPage className="text-foreground-400 text-[12px]/[16px] font-normal">
            {isMobile ? truncateString(txHash) : txHash}
          </BreadcrumbPage>
        ) : (
...
const TransactionHeader = memo(({ tx }: { tx: any }) => (
  <PageHeader
...
const StatusCard = memo(({ block, receipt }: { block: any; receipt: any }) => (
  <Card className="rounded-sm p-0">
    <CardContent className="flex-row justify-between">
      <div className="flex flex-col gap-[6px]">
        <CardLabel>Status</CardLabel>
        {block ? (
          <p className="inline-block text-[13px] font-semibold bg-gradient-to-r from-[#fff] to-[#636363] bg-clip-text text-transparent [text-shadow:0px_0px_10px_#6A6863]">
...
// Memoized server icon component
...
// Memoized transaction details component
const TransactionDetails = memo(({ 
  ...
  tx: any; 
  receipt: any; 
  onCopySenderAddress: (e: React.MouseEvent) => void; 
  onCopyValue: (value: string) => void; 
}) => {
  const hasSenderOrNonce = useMemo(() => 
    !!(tx?.sender_address || tx?.nonce), [tx?.sender_address, tx?.nonce]
  );

  const hasDataAvailabilityMode = useMemo(() => 
    !!(tx?.fee_data_availability_mode ?? tx?.nonce_data_availability_mode), 
    [tx?.fee_data_availability_mode, tx?.nonce_data_availability_mode]
  );

  const isVersion3 = useMemo(() => Number(tx?.version) === 3, [tx?.version]);
      ...
      <CardContent className="gap-[8px]">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex flex-row items-center gap-[10px]">
            <p className="text-[13px] font-mono font-medium text-foreground-200 max-w-xs break-all">
              {ConvertToSTRK(Number(cairo.felt(receipt?.actual_fee?.amount ?? 0))).toLocaleString()}
            </p>
            {receipt?.actual_fee && (
              <Badge className="uppercase bg-background-500 text-[10px]/[12px] font-medium px-[5px] py-[3px] pointer-events-none">
                strk
          ...
          <CardContent className="gap-[15px]">
            ...
            <div className="space-y-[8px]">
              ...
              <div className="flex items-center gap-px">
                <PriceCard
                  label="Max amount"
                  value={Number(cairo.felt(tx?.resource_bounds?.l1_gas?.max_amount ?? 0))}
                  unit="strk"
                  onClick={() => onCopyValue(tx?.resource_bounds?.l1_gas?.max_amount ?? "0")}
                />
                <PriceCard
                  label="Max price / unit"
                  value={ConvertToSTRK(Number(cairo.felt(tx?.resource_bounds?.l1_gas?.max_price_per_unit ?? 0)))}
                  unit="strk"
                  onClick={() => onCopyValue(tx?.resource_bounds?.l1_gas?.max_price_per_unit ?? "0")}
                />
              </div>
            </div>
            ...
            <div className="space-y-[8px]">
              ...
              <div className="flex items-center gap-px">
                <PriceCard
                  label="Max amount"
                  value={Number(cairo.felt(tx?.resource_bounds?.l2_gas?.max_amount ?? 0))}
                  unit="strk"
                  onClick={() => onCopyValue(tx?.resource_bounds?.l2_gas?.max_amount ?? "0")}
                />
                <PriceCard
                  label="Max price / unit"
                  value={ConvertToSTRK(Number(cairo.felt(tx?.resource_bounds?.l2_gas?.max_price_per_unit ?? 0)))}
                  unit="strk"
                  onClick={() => onCopyValue(tx?.resource_bounds?.l2_gas?.max_price_per_unit ?? "0")}
                />
              </div>
            </div>
            ...
            <div className="space-y-[8px]">
              ...
              <div className="flex items-center gap-px">
                <PriceCard
                  label="Max amount"
                  value={Number(cairo.felt(tx?.resource_bounds?.l1_data_gas?.max_amount ?? 0))}
                  unit="strk"
                  onClick={() => onCopyValue(tx?.resource_bounds?.l1_data_gas?.max_amount ?? "0")}
                />
                <PriceCard
                  label="Max price / unit"
                  value={ConvertToSTRK(Number(cairo.felt(tx?.resource_bounds?.l1_data_gas?.max_price_per_unit ?? 0)))}
                  unit="strk"
                  onClick={() => onCopyValue(tx?.resource_bounds?.l1_data_gas?.max_price_per_unit ?? "0")}
                />
              </div>
            </div>
          </CardContent>
...
const CalldataTab = memo(({ tx }: { tx: any }) => (
  <Suspense fallback={<Skeleton className="h-4 w-full" />}>
    <Calldata tx={tx} />
  </Suspense>
));

const ClassTab = memo(({ tx, declared }: { tx: any; declared: any }) => (
  <div className="flex flex-col gap-4">
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between gap-2">
        <CardLabel>Class Hash</CardLabel>
        <Hash value={tx.class_hash} to={`../class/${tx.class_hash}`} />
      </div>
      <div className="flex items-center justify-between gap-2">
...
const SignatureTab = memo(({ tx }: { tx: any }) => (
...
const EventsTab = memo(({ events }: { events: any }) => (
  <OptimizedDataTable table={events} />
));

const StorageDiffsTab = memo(({ storageDiff }: { storageDiff: any }) => (
  <OptimizedDataTable table={storageDiff} />
));

// Memoized tab list component
const TabList = memo(({ tx, tab }: { tx: any; tab: any }) => (
  <TabsList className="gap-[12px] p-0">
    {tx?.type === "INVOKE" && (
      ...
      >
        <ListIcon variant="solid" />
        <p className="text-[13px]/[20px] font-normal">Calldata</p>
      </TabsTrigger>
    )}
    {tx?.type === "DECLARE" && (
      <TabsTrigger
      ...
      >
        <ListIcon variant="solid" />
        <p className="text-[13px]/[20px] font-normal">Class</p>
      </TabsTrigger>
    )}
    <TabsTrigger
      value="signature"
    ...
    >
      ...
      <p className="text-[13px]/[20px] font-normal">Signature</p>
    </TabsTrigger>
    <TabsTrigger
      value="events"
      className="data-[state=active]:shadow-none gap-[4px] p-[8px]"
    ...
    >
      ...
      <p className="text-[13px]/[20px] font-normal">Storage Diffs</p>
    </TabsTrigger>
  </TabsList>
));
...
export function Transaction() {
  ...
  return (
    <div className="w-full flex flex-col gap-[3px] sl:w-[1134px]">
      <TransactionBreadcrumb txHash={txHash} isMobile={isMobile} />
      ...
      <div className="flex flex-col gap-[20px] lg:gap-[40px] pb-8">
        <div className="flex flex-col sl:flex-row sl:h-[73vh] gap-[3px]">
          <div className="sl:min-w-[337px] flex flex-col gap-[3px] sl:overflow-y-scroll">
            <StatusCard block={block} receipt={receipt} />
            <TransactionDetails 
              tx={tx} 
              receipt={receipt} 
              onCopySenderAddress={onCopySenderAddress} 
          ...
          <Card className="p-0 h-full flex-grow grid grid-rows-[min-content_1fr] overflow-x-scroll sl:min-w-[794px] rounded-sm rounded-br-[12px]">
            {tx ? (
              <Tabs value={tab.selected} onValueChange={tab.onChange} className="h-full">
                <CardContent className="pb-0 pt-[3px]">
                  <TabList tx={tx} tab={tab} />
                </CardContent>
                ...
                <CardContent className="p-[15px]">
                  ...
                  {tx?.type === "DECLARE" && !!declared && (
                    <TabsContent value="class" className="flex flex-col gap-4 mt-0">
                      <ClassTab tx={tx} declared={declared} />
                    </TabsContent>
                  )}
                  ...
                  <TabsContent value="storage-diffs" className="mt-0">
                    <StorageDiffsTab storageDiff={storageDiff} />
                  </TabsContent>
                </CardContent>
              </Tabs>
            ) : (
              <Tabs>
                <TabsList className="border-b border-background-200 animate-pulse pointer-events-none">
                  {Array.from({ length: 3 }).map((_, i) => (
                    <TabsTrigger key={i} value={`dummy-tab-${i}`}>
